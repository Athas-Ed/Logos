...
| `下一阶段开发计划.md` | 2026-05-12 定案的**广义下一阶段**队列（草案）；多数条目已在**第二阶段收尾**落实；**现行阶段排期**见 **`第四阶段开发计划.md`**。 |
| `第三阶段开发计划.md` | **重定向 stub**：第三阶段已结案（**P0** 已交付）；正文见 **`已完成/第三阶段开发计划.md`**。 |
| `第四阶段开发计划.md` | **第四阶段**主排期（**已定案**）：冻结优先级 **A7 → MCP → Obs**；契约 OpenAPI、A6、深度产品化、真壳 E2E **顺延下阶段**（见 **`重要子系统开发文档/产品化文档.md`**）。 |
