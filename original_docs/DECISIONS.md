...
| `重要子系统开发文档/` | 实现级长文；含 **`API-V0.2.md`**（现行 HTTP 契约）、**`API-V0.1.md`**（归档）、**`GUI开发文档.md`**（GUI + Electron 壳权威）等。 |
...
| `下一阶段开发计划.md` | 2026-05-12 定案的**广义下一阶段**队列（草案）；多数条目已在**第二阶段收尾**落实；**现行阶段排期**见 **`第三阶段开发计划.md`**。 |
| `第三阶段开发计划.md` | **第三阶段**主排期（已定案）：**P0 + P1**（Electron HA、A7、MCP 加固、可选契约工程）；**P2**（A6、docx Skill、自动更新等）后移。 |
