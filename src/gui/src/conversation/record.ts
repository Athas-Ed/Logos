...
import {
  ...
  LEGACY_CONVERSATION_SCHEMA_VERSION_V2,
...
function normalizeTaskPhase(raw: unknown): TaskPhase | undefined {
  if (raw === "input" || raw === "running") {
    return raw;
  }
  if (raw === "done") {
    return "input";
...
export function buildConversationRecord(args: {
  ...
  const record: ConversationRecord = {
    ...
    citation_turns: args.citationTurns,
    tool_trace_turns: args.toolTraceTurns,
    ...(args.reactStepLimitTurns && args.reactStepLimitTurns.length > 0 ?
      {
        react_step_limit_turns: args.reactStepLimitTurns.map((t) => ({
          hit: t.hit,
        })),
      }
    : {}),
...
function isCitationTurns(raw: unknown): raw is CitationItem[][] {
  if (!Array.isArray(raw)) {
    return false;
  }
  return raw.every((turn) => Array.isArray(turn) && turn.every(isCitation));
}

function isToolTraceTurns(raw: unknown): raw is string[][] {
  if (!Array.isArray(raw)) {
    return false;
  }
  return raw.every(
    (turn) =>
      Array.isArray(turn) && turn.every((x): x is string => typeof x === "string"),
  );
}
...
function parseCoreFieldsV3(
  ...
  let citation_turns: CitationItem[][] = [];
  if (raw.citation_turns !== undefined) {
    if (!isCitationTurns(raw.citation_turns)) {
      return null;
    }
    citation_turns = raw.citation_turns;
  } else {
    const legacy = Array.isArray(raw.citations) ? raw.citations : [];
    if (!legacy.every(isCitation)) {
      return null;
    }
    citation_turns = legacy.length > 0 ? [legacy as CitationItem[]] : [];
  }
  let tool_trace_turns: string[][] = [];
  if (raw.tool_trace_turns !== undefined) {
    if (!isToolTraceTurns(raw.tool_trace_turns)) {
      return null;
    }
    tool_trace_turns = raw.tool_trace_turns;
  } else {
    const legacy = Array.isArray(raw.tool_trace_log)
  ...
  return {
    ...
    citation_turns,
    tool_trace_turns,
    ...(Array.isArray(raw.react_step_limit_turns) ?
      {
        react_step_limit_turns: (
          raw.react_step_limit_turns as { hit?: unknown }[]
        ).map((t) => ({
          hit: t.hit === true,
        })),
      }
...
export function migrateConversationV1ToV2(
  ...
  const phase = normalizeTaskPhase(raw.task_phase);
  if (phase) {
...
export function parseConversationRecord(
  ...
  const core = parseCoreFieldsV3(id, raw);
  if (!core) {
    return null;
  }
  ...
  return attachSkillFields(core, raw);
}
