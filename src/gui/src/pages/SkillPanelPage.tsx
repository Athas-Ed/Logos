...
import { OFFLINE_SKILL_NAMES, type SkillCardMeta } from "../skills/catalog";
...
/** 离线时用极简标识组装 SkillCardMeta（只够面板展示卡片）。 */
function offlineSkillCards(): SkillCardMeta[] {
  return OFFLINE_SKILL_NAMES.map((n) => ({
    ...n,
    description: "",
    ui_instructions: "",
    paradigm: "dialogue" as const,
    persistence_tier: "p1" as const,
    turn_policy: "single" as const,
  }));
}
...
export function SkillPanelPage() {
  ...
  const [skills, setSkills] = useState<SkillCardMeta[]>(offlineSkillCards);
  ...
  return (
    <div className={styles.page} data-testid="skill-panel-page">
      ...
      <div className={styles.grid} role="list">
        {skills
          .filter((skill) => skill.panel_visible !== false)
          .map((skill) => (
