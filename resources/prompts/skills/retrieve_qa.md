...
1. **判断问题类型**：
   - 关系型（「住在」「携带」「同阵营」等）→ 先用 `kg_query(query_type="neighbors", ...)` 查图。
   - 事实型（「X 是什么」「X 的背景」）→ 先用 `retrieve` 搜正文。
   - 复合型 → 可以两路并行。
2. **kg_query 用法**：
   - `kg_query(query_type="neighbors", slug="罗德岛", max_hops=1)` → 返回该实体 1 跳邻居。
   - `kg_query(query_type="shortest_path", slug="罗德岛", target_slug="审判日组织")` → 返回两实体间最短路径。
   - `kg_query(query_type="neighbors", slug="悖论之地", relation_type="created_by")` → 只返回特定关系类型的邻居。
