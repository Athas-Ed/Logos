...
> **现阶段（2026-05-12）**：**不要求**在本目录新增「设定导入」MCP 包；设定导入流程见 **`docs/子系统文档/KSFS与叙事知识库.md`**。仍可用 **`example-stdio-mcp/`** 验证 MCP 与 S&G 接线。

## 与「设定导入」的关系（已定方向，实现延后）

- **用户触发路径**：导入流水线以 **「触发 Skill」** 为起点；**物理上**宜在 `skills/` 下新增独立目录（例如 `settings-import-mcp/`，名称实现期再定），内含 MCP 入口脚本、工具 schema、对 `entity_template` 的引用说明等。
